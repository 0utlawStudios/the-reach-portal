...
- Rechecked the live Settings > Audit Logs UI for the user-reported screenshot issue. Launch cleanup member removals render as `SYSTEM`.
- Found one remaining production QA artifact removal (`qa-invite-...@example.com`) that was generated by an automated test cleanup and still displayed Aldridge in metadata.
- Added a client-side audit actor guard for automated QA cleanup detail strings: `Removed qa-(invite|request)-<timestamp>@example.com from team, workspace access, and auth`.
- Added and applied migrations `0043_qa_cleanup_audit_actor_normalization.sql` and `0044_qa_cleanup_audit_actor_like_fix.sql`. The follow-up `LIKE` migration normalized the already-written production QA cleanup row after the first SQL regex proved too strict.
- Verified production `audit_log_v2` directly: QA cleanup and launch cleanup rows now have `metadata.user_name = SYSTEM`; the ordinary manual `themanekinekogirl@gmail.com` removal remains attributed to `aldridge@ten80ten.com`.
- Verified the live Audit Logs UI readback: `SYSTEM` appears immediately before the QA cleanup and launch cleanup `MEMBER REMOVED` rows; manual removal still shows Aldridge.
- Verification passed: `supabase db push --yes` for both migrations, focused setup static test, `git diff --check`, `npm run typecheck`, `npm run lint` with the existing AI worker warning only, full `npm test` with 29 files / 260 tests, and `npm run build`.
- Pushed functional commit `dc4d234` to `origin/main`.
...
Pipeline notification boundary hardening slice notes:

- Touched `src/lib/pipeline-context.tsx` only after rechecking the AGENTS iron-law requirements: provision still happens before posts SELECT, empty DB arrays remain valid, every post insert still carries `workspace_id`, and UUID guards remain in place for id-keyed writes.
- Fixed the create-card realtime race: when a Supabase INSERT echo arrives before the insert promise resolves, the temp card and real UUID are now reconciled idempotently instead of leaving duplicate cards.
- Added `src/app/api/notifications/_shared.ts` so approved, awaiting-approval, revision, and mention notification routes all require an active workspace member via `requireBearerTeamRole`.
- Hardened notification routes to validate post UUIDs and load the canonical post by `id + workspace_id` before sending email or writing audit rows.
- Notification routes now derive actor display names and creator/recipient data server-side from `team_members`; browser-supplied `postTitle`, `createdBy`, `approvedBy`, `movedBy`, `requestedBy`, `authorName`, and `authorEmail` are no longer trusted as authoritative.
