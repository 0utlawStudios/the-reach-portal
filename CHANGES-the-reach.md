...
Latest slice: pipeline notification boundary hardening is pushed as `03218aa`. Create-card temp/real UUID reconciliation now deduplicates realtime insert races, and notification APIs now require active workspace access, verify the post by workspace, derive actors/recipients from server-side team data, and write workspace-scoped audit rows. Verification passed focused pipeline/notification tests, `git diff --check`, `npm run typecheck`, `npm run lint`, full `npm test`, and `npm run build`.
